...
from ...utils.loss_utils import l1_loss, lpips, smooth_l1_loss, ssim
...
class SLatVaeMeshDecoderTrainer(BasicTrainer):
    ...
    def _init_renderer(self):
        rendering_options = {"near": 1, "far": 3}
    ...
    def _perceptual_loss(self, gt: torch.Tensor, pred: torch.Tensor, name: str) -> Dict[str, torch.Tensor]:
        ...
        terms[f"{name}_loss_perceptual"] = (
            terms[f"{name}_loss"]
            + terms[f"{name}_loss_ssim"] * self.lambda_ssim
            + terms[f"{name}_loss_lpips"] * self.lambda_lpips
        )
        return terms
        ...
        if success_mask.sum() != 0:
            ...
            if self.depth_loss_type == "l1":
                ...
                terms["depth_loss"] = smooth_l1_loss(
                    buffer["depth"] * target["mask"], target["depth"] * target["mask"], beta=1.0 / (2 * reps[0].res)
                )
            else:
                raise ValueError(f"Unsupported depth loss type: {self.depth_loss_type}")
            terms.update(
                self._perceptual_loss(buffer["normal"] * target["mask"], target["normal"] * target["mask"], "normal")
            )
            terms["geo_loss"] = (
                terms["geo_loss"]
                + terms["mask_loss"]
                + terms["depth_loss"] * self.lambda_depth
                + terms["normal_loss_perceptual"]
            )
    ...
    def color_losses(self, reps, image, alpha, extrinsics, intrinsics):
        ...
        if success_mask.sum() != 0:
            terms.update(
                self._perceptual_loss((image * alpha[:, None])[success_mask], buffer["color"][success_mask], "color")
            )
    ...
    def run_snapshot(
        ...
        ret_dict.update(
            {f"gt_normal": {"value": self._flip_normal(gt_render_results["normal"], exts, ints), "type": "image"}}
        )
        return_types = ["normal"]
        if self.use_color:
