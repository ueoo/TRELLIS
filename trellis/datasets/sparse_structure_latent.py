...
class SparseStructureLatentVisMixin:
    def __init__(
        ...
        **kwargs,
        ...
        self.renderer_resolution = 512
        self.renderer_near = 0.8
        self.renderer_far = 1.6
        self.renderer_bg_color = (0, 0, 0)
        self.renderer_ssaa = 4
        self.renderer_primitive = "voxel"
        self.renderer_cam_orig_scale = 2.0
    ...
    def visualize_sample(self, x_0: Union[torch.Tensor, dict]):
        ...
        renderer.rendering_options.resolution = self.renderer_resolution
        renderer.rendering_options.near = self.renderer_near
        renderer.rendering_options.far = self.renderer_far
        renderer.rendering_options.bg_color = self.renderer_bg_color
        renderer.rendering_options.ssaa = self.renderer_ssaa
        ...
        for i in range(x_0.shape[0]):
            ...
            for j, (ext, intr) in enumerate(zip(exts, ints)):
                ...
                image[
                    :, 512 * (j // tile[1]) : 512 * (j // tile[1] + 1), 512 * (j % tile[1]) : 512 * (j % tile[1] + 1)
                ] = res["color"]
            images.append(image)
...
class SparseStructureLatentConditionedSparseStructureLatent(
    SparseStructureLatentConditionedMixin, SparseStructureLatent
):
    """
    Sparse-structure-latent-conditioned sparse structure dataset
    """

    pass


class PrevImageConditionedSparseStructureLatent(PrevImageConditionedMixin, SparseStructureLatent):
    """
    Prev-image-conditioned sparse structure dataset
    """

    pass


class PrevImageCondConditionedSparseStructureLatent(PrevImageCondConditionedMixin, SparseStructureLatent):
    """
    Prev-image-conditioned sparse structure dataset
    """
