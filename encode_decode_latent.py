...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dec_pretrained",
        type=str,
        default="microsoft/TRELLIS-image-large/ckpts/slat_dec_mesh_swin8_B_64l8m256c_fp16",
        help="Pretrained encoder model",
    )
    ...
    os.makedirs(os.path.join(opt.output_dir, "latents"), exist_ok=True)
    os.makedirs(os.path.join(opt.output_dir, "latents_gt_vis"), exist_ok=True)
    os.makedirs(os.path.join(opt.output_dir, "latents_recon_vis"), exist_ok=True)
    ...
    for sha256 in copy.copy(sha256s):
        latent_path = os.path.join(opt.output_dir, "latents", f"{sha256}.npz")
        latent_gt_vis_path = os.path.join(opt.output_dir, "latents_gt_vis", f"{sha256}.jpg")
        latent_recon_vis_path = os.path.join(opt.output_dir, "latents_recon_vis", f"{sha256}.jpg")
        # if os.path.exists(latent_path) and os.path.exists(latent_gt_vis_path) and os.path.exists(latent_recon_vis_path):
        ...
        with ThreadPoolExecutor(max_workers=32) as loader_executor, ThreadPoolExecutor(
            ...
            def loader(sha256):
                try:
                    ...
                    print(feats.keys())
                    print(feats["patchtokens"].shape)
                    print(feats["indices"].shape)
            ...
            for _ in trange(len(sha256s), desc="Extracting latents"):
