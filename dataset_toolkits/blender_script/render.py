...
def scale_scene(scale: float) -> Tuple[float, Vector]:
    """Normalizes the scene by scaling and translating it to fit in a unit cube centered
    at the origin.
...
def main(arg):
    ...
    if not os.path.exists(json_file) or num_rendered_frames < len(json.loads(arg.views)):
        ...
        for i, view in enumerate(views):
            ...
            out_file_path = os.path.join(arg.output_folder, f"{i:03d}.png")
            bpy.context.scene.render.filepath = out_file_path
    ...
    if arg.save_mesh:
        ...
        if not os.path.exists(mesh_file_path):
            ...
            # export ply mesh
            bpy.ops.wm.ply_export(filepath=mesh_file_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Renders given obj file by rotation a camera around it.")
    parser.add_argument(
        "--views", type=str, help="JSON string of views. Contains a list of {yaw, pitch, radius, fov} object."
    )
