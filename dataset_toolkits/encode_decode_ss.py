...
import trellis.datasets as datasets
...
if __name__ == "__main__":
    ...
    if opt.dec_model is None:
        latent_name = f'{opt.dec_pretrained.split("/")[-1]}'
        decoder = models.from_pretrained(opt.dec_pretrained).eval().cuda()
    else:
        latent_name = f"{opt.dec_model}_{opt.ckpt}"
        cfg = edict(json.load(open(os.path.join(opt.model_root, opt.dec_model, "config.json"), "r")))
        decoder = getattr(models, cfg.models.decoder.name)(**cfg.models.decoder.args).cuda()
        ckpt_path = os.path.join(opt.model_root, opt.dec_model, "ckpts", f"decoder_{opt.ckpt}.pt")
        decoder.load_state_dict(torch.load(ckpt_path), strict=False)
        decoder.eval()
        print(f"Loaded model from {ckpt_path}")

    os.makedirs(os.path.join(opt.output_dir, "ss_latents"), exist_ok=True)
    os.makedirs(os.path.join(opt.output_dir, "ss_gt"), exist_ok=True)
    os.makedirs(os.path.join(opt.output_dir, "ss_decoded"), exist_ok=True)
    ...
    ss_vis = datasets.SparseStructure(roots=opt.output_dir)

    # filter out objects that are already processed
    sha256s = list(metadata["sha256"].values)
    for sha256 in copy.copy(sha256s):
        latent_path = os.path.join(opt.output_dir, "ss_latents", f"{sha256}.npz")
        gt_path = os.path.join(opt.output_dir, "ss_gt", f"{sha256}.jpg")
        decoded_path = os.path.join(opt.output_dir, "ss_decoded", f"{sha256}.jpg")
        # if os.path.exists(latent_path) and os.path.exists(decoded_path):
        #     records.append({"sha256": sha256, f"ss_latent_{latent_name}": True, f"ss_decoded_{latent_name}": True})
    ...
    try:
        with ThreadPoolExecutor(max_workers=32) as loader_executor, ThreadPoolExecutor(
            ...
            def saver(sha256, pack):
                save_path = os.path.join(opt.output_dir, "ss_latents", f"{sha256}.npz")
                np.savez_compressed(save_path, **pack)
                records.append({"sha256": sha256, f"ss_latent_{latent_name}": True})

            for _ in trange(len(sha256s), desc="Encoding and decoding"):
                sha256, ss = load_queue.get()
                ss = ss.cuda().float()
                latent = encoder(ss, sample_posterior=False)
                logits = decoder(latent)
                recon = (logits > 0).long()
                gt_image = ss_vis.visualize_sample(ss)
                recon_image = ss_vis.visualize_sample(recon)

                utils.save_image(
                    gt_image,
                    os.path.join(opt.output_dir, "ss_gt", f"{sha256}.jpg"),
                    nrow=1,
                    normalize=True,
                    value_range=ss_vis.value_range,
                )

                utils.save_image(
                    recon_image,
                    os.path.join(opt.output_dir, "ss_decoded", f"{sha256}.jpg"),
                    nrow=1,
                    normalize=True,
                    value_range=ss_vis.value_range,
                )
