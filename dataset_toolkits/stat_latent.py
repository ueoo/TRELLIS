...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, default="dinov2_vitl14_reg_slat_enc_swin8_B_64l8_fp16", help="Latent model to use"
    )
    parser.add_argument(
        "--num_samples", type=int, default=50000, help="Number of samples to use for calculating stats"
    )
    ...
    means = []
    mean2s = []
    with ThreadPoolExecutor(max_workers=16) as executor, tqdm(total=len(sha256s), desc="Extracting features") as pbar:
