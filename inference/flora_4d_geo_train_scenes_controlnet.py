...
def main(args):
    ...
    finetune_name = "flora4dgeo-pretrainedvae-fullflow-controlnet"
    ...
    train_data_root = "/scr/yuegao/TRELLIS_datasets/Flora125Geo_train_merged"
    ...
    if args.world_size > 1:
        start = len(train_scene_names) * args.rank // args.world_size
        end = len(train_scene_names) * (args.rank + 1) // args.world_size
        train_scene_names = train_scene_names[start:end]
    # Further split across GPUs
    train_scene_names = train_scene_names[args.gpu_idx :: args.gpu_num]

    num_frames = args.num_frames

    for scene_name in tqdm(
        train_scene_names,
        total=len(train_scene_names),
        ...
        cond_frame_path = os.path.join(
            train_data_root, "renders_fixview", f"{scene_name}_{args.first_frame:04d}", "000.png"
        )
        ...
        sub_folder = f"sample_gs_{num_frames}_train_scene_{scene_name}_frame_{args.first_frame:04d}"
