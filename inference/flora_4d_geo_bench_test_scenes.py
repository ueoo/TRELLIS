...
def main(args):
    ...
    output_folder = os.path.join(results_root, f"results_gtprevcond_{finetune_name.replace('-', '_')}")
    print(f"Saving outputs to {output_folder}")

    infer_data_root = "/scr/yuegao/TRELLIS_datasets/Flora125Geo_test_merged"
    ...
    for scene_name in tqdm(
        ...
        # SLAT metrics
        plot_series("slat_mean_l2", get_vals("mean_l2"), "SLAT mean L2")
        plot_series("slat_mean_cosine_distance", get_vals("mean_cosine_distance"), "SLAT mean cosine distance")
        plot_series("slat_match_ratio_gen", get_vals("match_ratio_gen"), "SLAT match ratio (gen)")
        plot_series("slat_match_ratio_gt", get_vals("match_ratio_gt"), "SLAT match ratio (gt)")
