...
def main(args):
    ...
    finetune_name = "flora3dgeo-pretrainedvae-fullflow"
    ...
    for scene_name in tqdm(
        ...
        all_videos = []
        sub_folder = f"sample_gs_{num_frames}_test_scene_{scene_name}"
        os.makedirs(os.path.join(output_folder, sub_folder), exist_ok=True)

        for frame_idx in trange(
            args.first_frame, args.first_frame + num_frames, desc="Processing frames", leave=False
        ):
            # Load an image
            cond_frame_path = os.path.join(
                test_data_root, "renders_fixview", f"{scene_name}_{frame_idx:04d}", "000.png"
            )
            ...
            input_frame_name = f"input_scene_{scene_name}_frame_{frame_idx:04d}.png"
        ...
        for view_idx in trange(num_views, desc="Saving views", leave=False):
            cur_view_videos = [all_videos[frame_idx][view_idx] for frame_idx in range(len(all_videos))]
            imageio.mimsave(
